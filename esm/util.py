...
from esm.entities import TokenSpan
...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        ...
        if 'rel_classifier.weight' in state_dict and loaded_version != model_class.VERSION:
            msg = ("Current SpERT version (%s) does not match the version of the loaded esm (%s). "
                   % (model_class.VERSION, loaded_version))
            msg += "Use the code matching your version or train a new esm."
            raise Exception(msg)
