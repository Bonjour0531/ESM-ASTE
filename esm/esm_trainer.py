...
from esm import models
from esm import sampling
from esm import util
from esm.entities import Dataset
from esm.evaluator import Evaluator
from esm.input_reader import JsonInputReader, BaseInputReader
from esm.loss import ESMLoss, Loss
from tqdm import tqdm
from esm.trainer import BaseTrainer

SCRIPT_PATH = os.path.dirname(os.path.realpath(__file__))


class ESMTrainer(BaseTrainer):
    """ Joint terms and sentiment polarity extraction training and evaluation """
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        # ESM is currently optimized on a single GPU and not thoroughly tested in a multi GPU setup
        # If you still want to train ESM on multiple GPUs, uncomment the following lines
        # # parallelize esm
        # if self._device.type != 'cpu':
        #     esm = torch.nn.DataParallel(esm)
        ...
        compute_loss = ESMLoss(rel_criterion, entity_criterion, model, optimizer, scheduler, args.max_grad_norm)
        ...
        for epoch in range(args.epochs):
            ...
            if not args.final_eval or (epoch == args.epochs - 1):
                ...
                print(f'here is f1:{f1}')
                if f1 > max_f1 and epoch > 5:
                    max_f1 = f1
                    # save the best f1 esm
        ...
        # extra = dict(epoch=args.epochs, updates_epoch=updates_epoch, epoch_iteration=0)
        # global_iteration = args.epochs * updates_epoch
        # self._save_model(self._save_path, esm, self._tokenizer, global_iteration,
        #                  optimizer=optimizer if self.args.save_optimizer else None, extra=extra,
    ...
    def eval(self, dataset_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        # evaluate
        f1 = self._eval(model, input_reader.get_dataset(dataset_label), input_reader)
