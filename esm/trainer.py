...
from esm.opt import tensorboardX
...
class BaseTrainer:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self._log_path = os.path.join(self.args.log_path, self.args.label)
        ...
        # CUDA devices
        self._device = torch.device("cuda:0" if torch.cuda.is_available() and not args.cpu else "cpu")
    ...
    def _save_best(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, optimizer: Optimizer,
        ...
        if accuracy > self._best_results[label]:
            self._logger.info("[%s] Best esm in iteration %s: %s%% accuracy" % (label, iteration, accuracy))
    ...
    def _save_model(self, save_path: str, model: PreTrainedModel, tokenizer: PreTrainedTokenizer,
                    ...
                    extra: dict = None, include_iteration: int = True, name: str = 'esm'):
