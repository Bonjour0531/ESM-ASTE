...
class ESM(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, cls_token: int, relation_types: int, entity_types: int,
        ...
        super(ESM, self).__init__(config)
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
        ...
        # h, cls = self.bert(input_ids=encodings, attention_mask=context_masks)
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        # entity_mask: [batch, sample_num, seq_len], [16, 104, 63]
        # max pool entity candidate spans
        m = (entity_masks.unsqueeze(-1) == 0).float() * (-1e30)
        # [16, 104, 63, 1]
        entity_spans_pool = m + h.unsqueeze(1).repeat(1, entity_masks.shape[1], 1, 1)
        # after mask: the area masked is 负无穷小
        # [16, 104, 63, 768]
        entity_spans_pool = entity_spans_pool.max(dim=2)[0]

        '''
        # average pool entity candidate spans
        m = (entity_masks.unsqueeze(-1) == 1).float()
        entity_spans_pool = m * h.unsqueeze(1).repeat(1, entity_masks.shape[1], 1, 1)
    ...
    def _classify_relations(self, entity_spans, size_embeddings, relations, rel_masks, h, chunk_start):
        ...
        '''
        # full context
        full_ctx = h.max(dim=2)[0]
        rel_repr = torch.cat([full_ctx, entity_pairs, size_pair_embeddings], dim=2)
        '''

        '''
        # cls context
        cls_ctx = h[:, :, 0, :]
        rel_repr = torch.cat([cls_ctx, entity_pairs, size_pair_embeddings], dim=2)
        '''
