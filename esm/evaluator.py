...
from esm import util
from esm.entities import Document, Dataset, EntityType
from esm.input_reader import JsonInputReader
from esm.opt import jinja2
...
class Evaluator:
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
        ...
        for i in range(batch_size):
            # get esm predictions for sample
            ...
            rel_types = (rel_nonzero % rel_class_count) + 1  # esm does not predict None class (+1)
    ...
    def compute_scores(self):
        ...
        ner_eval, ner_ans= self._score(gt, pred, print_results=True)
        ...
        rel_eval, rel_ans= self._score(gt, pred, print_results=True)
        ...
        rel_nec_eval, rel_nec_ans = self._score(gt, pred, print_results=True)

        if self._dataset_label == 'test':
            if os.path.exists('./result/save_result.json'):
                with open('./result/save_result.json', 'r', encoding='utf-8') as f:
                    save_result = json.load(f)
            else:
                save_result = []
            temp = {}
            temp['opinion'] = (float(ner_ans['opinion'][1]), float(ner_ans['opinion'][2]), float(ner_ans['opinion'][3]))
            temp['aspect'] = (float(ner_ans['aspect'][1]), float(ner_ans['aspect'][2]), float(ner_ans['aspect'][3]))
            temp['rel_micro'] = (float(rel_nec_ans['micro'][1]), float(rel_nec_ans['micro'][2]), float(rel_nec_ans['micro'][3]))
            save_result.append(temp)
            print(save_result)
            with open('./result/save_result.json', 'w', encoding='utf-8') as f:
    ...
    def _score(self, gt: List[List[Tuple]], pred: List[List[Tuple]], print_results: bool = False):
        ...
        metrics, ans = self._compute_metrics(gt_flat, pred_flat, types, print_results)
        return metrics, ans
    ...
    def _compute_metrics(self, gt_all, pred_all, types, print_results: bool = False):
        ...
        ans, result_str = self._print_results(per_type, list(micro) + [total_support], list(macro) + [total_support], types)
        if print_results:
            print(result_str)
        return [m * 100 for m in micro + macro], ans
    ...
    def _print_results(self, per_type: List, micro: List, macro: List, types: List):
        ...
        for m, t in zip(metrics_per_type, types):
            ...
            ans[t.short_name] = self._get_row(m, t.short_name)
            results.append('\n')
        ...
        results.append('\n')
        ans['micro'] = self._get_row(micro, 'micro')
    ...
    def _convert_example(self, doc: Document, gt: List[Tuple], pred: List[Tuple],
        ...
        if gt or pred:
            ...
            temp, _ = self._score([gt], [pred_s])
            precision, recall, f1= temp[:3]
        else:
