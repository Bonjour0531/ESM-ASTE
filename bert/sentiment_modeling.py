...
def get_span_representation(span_starts, span_ends, input, input_mask):
    '''
    :param span_starts: [N, M]   batch, max_term_num
    :param span_ends: [N, M]
    :param input: [N, L, D]
...
class BertForBIOAspectAndOpinionExtraction(nn.Module):
    def __init__(self, config, use_crf=True):
        super(BertForBIOAspectAndOpinionExtraction, self).__init__()
        self.bert = BertModel(config)
        self.use_crf = use_crf
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.affine = nn.Linear(config.hidden_size, 9)
        if self.use_crf:
            self.crf = ConditionalRandomField(9)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, bio_labels=None, device=None):
        all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
        sequence_output = all_encoder_layers[-1]
        logits = self.affine(sequence_output)   # [N, L, 11]
...
class BertForSequenceClassification(nn.Module):
    """BERT esm for classification.
    This module is composed of the BERT esm with a linear layer on top of
    the pooled output.

    Params:
        `config`: a BertConfig class instance with the configuration to build a new esm.
        `num_labels`: the number of classes for the classifier. Default = 2.

    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary(see the tokens preprocessing logic in the scripts
            `extract_features.py`, `run_questionsbert.py` and `run_squad.py`)
    ...
    esm = BertForSequenceClassification(config, num_labels)
    logits = esm(input_ids, token_type_ids, input_mask)
    ```
    """
...
class BertForSentimentClsWithLc(nn.Module):

    def __init__(self, config, num_labels=2):
        super(BertForSentimentClsWithLc, self).__init__()
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size*3 + 25*2, num_labels)
        self.size_embeddings = nn.Embedding(100, 25)  # length_embedding
    ...
    def get_term_repr(self, aspects, h):
        at_m = ((aspects == 0).float() * (-1e30)).unsqueeze(-1)
        at_repr = at_m + h         # batch, len, dim
        # max pooling
        at_repr = at_repr.max(dim=1)[0]  # batch, dim
        return at_repr
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                labels=None, aspects=None, opinions=None, local_context=None, at_size=None, ot_size=None):
        h_all_layer, pooled_output = self.bert(input_ids, token_type_ids, attention_mask)
        h = h_all_layer[-1]
        pooled_output = self.dropout(pooled_output)

        # get length embedding
        at_length_emb = self.size_embeddings(at_size).squeeze(1)  # batch, dim(25)
        ot_length_emb = self.size_embeddings(ot_size).squeeze(1)  # batch, dim(25)

        aspects = aspects.squeeze(1)
        opinions = opinions.squeeze(1)
        local_context = local_context.squeeze(1)

        # get aspect repr, opinion repr, local_context repr
        at_repr = self.get_term_repr(aspects, h)       # batch, dim
        ot_repr = self.get_term_repr(opinions, h)      # batch, dim
        lc_repr = self.get_term_repr(local_context, h) # batch, dim

        at_repr[aspects.to(torch.uint8).any(-1) == 0] = 0
        ot_repr[opinions.to(torch.uint8).any(-1) == 0] = 0
        lc_repr[local_context.to(torch.uint8).any(-1) == 0] = 0

        # final relation repr
        rel_repr = torch.cat([lc_repr, at_repr, ot_repr, at_length_emb, ot_length_emb], dim=-1)
...
class BertForSpanAspectExtraction(nn.Module):
    """BERT esm for Question Answering (span extraction).
    This module is composed of the BERT esm with a linear layer on top of
    ...
    esm = BertForQuestionAnswering(config)
    start_logits, end_logits = esm(input_ids, token_type_ids, input_mask)
    ```
    """
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, start_positions=None, end_positions=None):
        all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
        sequence_output = all_encoder_layers[-1]  # batch, max_seq_len, dim
        logits = self.qa_outputs(sequence_output) # batch, max_seq_len, 2
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1) # batch, max_seq_len
        end_logits = end_logits.squeeze(-1) # batch, max_seq_len
